...
const ClientManagement = ({ currentUser, showToast }) => {
    ...
    const saveClient = async (e) => {
        e.preventDefault();
        try {
            if (editingClient) { await api.put(`/clients/${editingClient._id}`, form); showToast?.('Client records synchronized', 'success'); }
            else { await api.post('/clients', form); showToast?.('New client identity registered', 'success'); }
    ...
    const deleteClient = (id) => {
        setConfirmAction({
            message: "Permanently terminate this client profile? This will eliminate all associated historical metadata.",
            onConfirm: async () => {
                try { await api.delete(`/clients/${id}`); showToast?.('Identity eliminated', 'success'); fetchClients(); }
                ...
                <div style={cardStyle}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
                        ...
                        <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
                            ...
                            <motion.button whileTap={{ scale: 0.95 }} title="Register a new Client Identity" onClick={() => openModal()} style={{ background: '#0f172a', color: '#fff', border: 'none', borderRadius: '12px', padding: '0.8rem 1.5rem', fontWeight: 800, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.5rem' }}><Plus size={18} /> Add Client</motion.button>
                        </div>
                    </div>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                        <thead>
                            <tr style={{ borderBottom: '2px solid #f1f5f9' }}>
                                ...
                                <th style={{ padding: '1.25rem 1rem', color: '#64748b', fontWeight: 800, textTransform: 'uppercase', fontSize: '0.75rem' }}>Entity Type</th>
                                ...
                                <tr key={c._id} style={{ borderBottom: '1px solid #f1f5f9', transition: 'background 0.2s' }}>
                                    ...
                                    <td style={{ padding: '1.25rem 1rem', textAlign: 'right' }}>
                                        <div style={{ display: 'flex', gap: '0.4rem', justifyContent: 'flex-end' }}>
                                            <motion.button whileTap={{ scale: 0.9 }} title="View Identity Details" onClick={() => setViewClient(c)} style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '8px', padding: '8px 12px', cursor: 'pointer', fontWeight: 800, color: '#0f172a', display: 'flex', gap: '0.4rem', fontSize: '0.7rem' }}><FileText size={12} /> View</motion.button>
                                            <motion.button whileTap={{ scale: 0.9 }} title="Update Identity Records" onClick={() => openModal(c)} style={{ background: '#eff6ff', border: '1px solid #dbeafe', borderRadius: '8px', padding: '8px 12px', cursor: 'pointer', fontWeight: 800, color: '#2563eb', display: 'flex', gap: '0.4rem', fontSize: '0.7rem' }}><Edit2 size={12} /> Edit</motion.button>
                                            <motion.button whileTap={{ scale: 0.9 }} title="Terminate Identity" onClick={() => deleteClient(c._id)} style={{ background: '#fff1f2', border: '1px solid #fee2e2', borderRadius: '8px', padding: '8px 12px', cursor: 'pointer', fontWeight: 800, color: '#ef4444', fontSize: '0.7rem' }}>Delete</motion.button>
                                        </div>
                                    </td>
                                </tr>
                            ))}
                ...
                {viewClient && (
                    <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.6)', backdropFilter: 'blur(10px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
                        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} style={{ background: '#fff', borderRadius: '32px', padding: '3rem', width: '100%', maxWidth: 500, boxShadow: '0 50px 100px -20px rgba(0,0,0,0.4)' }}>
                            ...
                            <motion.button whileTap={{ scale: 0.98 }} onClick={() => setViewClient(null)} style={{ background: '#0f172a', color: '#fff', border: 'none', borderRadius: '14px', width: '100%', padding: '1rem', fontWeight: 800, cursor: 'pointer', marginTop: '2rem' }}>Close Inspector</motion.button>
                        </motion.div>
                    </div>
                )}
            </AnimatePresence>
                    ...
                    <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.4)', backdropFilter: 'blur(10px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
                        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} style={{ background: '#fff', borderRadius: '32px', padding: '3rem', width: '100%', maxWidth: 600, boxShadow: '0 50px 100px -20px rgba(0,0,0,0.3)', maxHeight: '90vh', overflowY: 'auto' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '2.5rem' }}>
                                <h2 style={{ margin: 0, fontWeight: 900, fontSize: '1.5rem' }}>{editingClient ? 'Sync Identity' : 'Register Identity'}</h2>
                            ...
                            <form onSubmit={saveClient}>
                                ...
                                <div style={{ background: '#f8fafc', padding: '2rem', borderRadius: '24px', border: '1px solid #e2e8f0', marginBottom: '2rem' }}>
                                    <h4 style={{ margin: '0 0 1.5rem 0', fontWeight: 800, color: '#0f172a' }}>Contact Matrix</h4>
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                                        ...
                                        <div><label style={labelStyle}>WhatsApp Link</label><input title="WhatsApp handle" value={form.whatsappNumber} onChange={e => setForm({...form, whatsappNumber: e.target.value})} style={{...inputStyle, background: '#fff'}} /></div>
                                        ...
                                        <div style={{ gridColumn: '1 / -1' }}><label style={labelStyle}>Physical Domicile</label><input title="Geographic location" value={form.address} onChange={e => setForm({...form, address: e.target.value})} style={{...inputStyle, background: '#fff'}} /></div>
                                    </div>
                                </div>
                                <div style={{ display: 'flex', gap: '1rem' }}>
                                    <motion.button whileTap={{ scale: 0.98 }} title="Commit changes to repository" type="submit" style={{ background: '#10b981', color: '#fff', border: 'none', borderRadius: '14px', width: '100%', padding: '1rem', fontWeight: 800, cursor: 'pointer', justifyContent: 'center', display: 'flex' }}>COMMIT SYNCHRONIZATION</motion.button>
                                </div>
                            </form>
                        </motion.div>
                    </div>
                ...
                {confirmAction && (
                    <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.6)', backdropFilter: 'blur(10px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000}}>
                        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} style={{ background: '#fff', borderRadius: '32px', padding: '3rem', width: '100%', maxWidth: 450, textAlign: 'center', boxShadow: '0 50px 100px -20px rgba(0,0,0,0.4)' }}>
                            ...
                            <h3 style={{ margin: 0, fontWeight: 900, fontSize: '1.5rem', color: '#0f172a' }}>Confirm Termination?</h3>
                            <p style={{ color: '#64748b', marginTop: '1rem', marginBottom: '2.5rem', lineHeight: '1.6', fontWeight:600 }}>{confirmAction.message}</p>
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                                <motion.button whileTap={{ scale: 0.95 }} title="Abort Termination" onClick={() => setConfirmAction(null)} style={{ padding: '1rem', borderRadius: '16px', border: '1.5px solid #e2e8f0', background: '#fff', cursor: 'pointer', fontWeight: 800, color: '#0f172a' }}>Abort</motion.button>
                                <motion.button whileTap={{ scale: 0.95 }} title="Execute Elimination" onClick={() => { confirmAction.onConfirm(); setConfirmAction(null); }} style={{ padding: '1rem', borderRadius: '16px', border: 'none', background: '#ef4444', color: '#fff', cursor: 'pointer', fontWeight: 800 }}>Execute</motion.button>
                            </div>
                        </motion.div>
                    </div>
                )}
